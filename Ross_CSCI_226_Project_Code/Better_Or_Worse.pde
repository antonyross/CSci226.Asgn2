...
void setup() {
  ...
  smooth();
  font = loadFont("/Users/antonyr/Desktop/Processing_Charts/Better_Or_Worse/data/Verdana-Bold-14.vlw");
  errorFont = loadFont("/Users/antonyr/Desktop/Processing_Charts/Better_Or_Worse/data/Verdana-Bold-20.vlw");
  textFont(font);
  
  engine.addParticle(new Particle("J. Blake", random(0, 350), random(0, 350), 0, 0));
  backupTable = new Table("/Users/antonyr/Desktop/Processing_Charts/Better_Or_Worse/data/following.tsv");
  //try {
    ...
    for(int i = 0; i < 33/*ids.getIDs().length*/; i++) {
      ...
      engine.connectParticles(name, "J. Blake");
    }
  //} catch(TException e) {
    //println("Error: "+e);
  //}
  
  engine.findParticle("J. Blake").pin(width/2, height/2);
}
